      ...
      if  (intoar or mid):# ori intoarcere ori mid ca plm te poti astepta la orice fel de noise
       ...
       # self.decizie=self.decizie+1
        ...
        if self.ok==2: ############### PRE-LANE (pozitionare ######################
          ...
          if (fs.StangaLim>fs.width/2-30 or fs.StangaLim1> fs.width/2-30 or fs.DreaptaLim<fs.width/2+30 or fs.DreaptaLim1<fs.width/2+30) and self.decizie==1:
            self.okdeintoar=0
            #self.decizie=1
            self.command.linear.x=0.1 #si daca se fute intoarcerea , o redresam oleaca
            self.command.angular.z = 0.4
            self.pub.publish(self.command)
          elif(fs.StangaLim>fs.width/2-30 or fs.StangaLim1> fs.width/2-30 or fs.DreaptaLim<fs.width/2+30 or fs.DreaptaLim1<fs.width/2+30) and self.decizie==0:
            #self.decizie=0  
            self.command.linear.x=0.1 #si daca se fute intoarcerea , o redresam oleaca
            self.command.angular.z = -0.4
            self.pub.publish(self.command)
        ...
        #self.ok=0
          ...
          self.command.angular.z = 6*(target_rad-yaw) #unghiul la care sa se roteasa
          self.command.linear.x=1 #viteza de mers in fata
          self.pub.publish(self.command)
